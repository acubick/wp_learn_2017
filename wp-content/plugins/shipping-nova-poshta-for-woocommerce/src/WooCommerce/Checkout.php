...
use NovaPoshta\Pro\WooCommerce\Shipping\Methods\Courier\Courier;
...
class Checkout {
	...
	private function validate_shipping_method() {
		...
		if ( isset( $_POST['shipping_nova_poshta_for_woocommerce_city'] ) && empty( $_POST['shipping_nova_poshta_for_woocommerce_city'] ) ) { // phpcs:ignore
			wc_add_notice( __( 'Select delivery city', 'shipping-nova-poshta-for-woocommerce' ), 'error' );
		}

		if ( isset( $_POST['shipping_nova_poshta_for_woocommerce_warehouse'] ) && empty( $_POST['shipping_nova_poshta_for_woocommerce_warehouse'] ) ) { // phpcs:ignore
