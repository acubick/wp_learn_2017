...
class License {
	...
	public function controller() {
		...
		if ( Main::PLUGIN_SLUG !== $page ) {
			return;
		}

		$nonce   = filter_input( INPUT_POST, '_wpnonce', FILTER_SANITIZE_STRING );
		$method  = filter_input( INPUT_POST, 'method', FILTER_SANITIZE_STRING );
		$api_key = filter_input( INPUT_POST, 'license_key', FILTER_SANITIZE_STRING );

		if ( ! wp_verify_nonce( $nonce, Main::PLUGIN_SLUG . '-license' ) || empty( $api_key ) ) {
			return;
		}
	...
	public function validate() {
		...
		$this->notice->add(
			'error',
			esc_html__( 'Your license key is invalid', 'shipping-nova-poshta-for-woocommerce' )
		);
	}
	...
	public function is_valid_license(): bool {
		...
		if ( ! empty( $response['status_check'] ) && 'active' === $response['status_check'] ) {
			$cache->set( 'license', true, 12 * HOUR_IN_SECONDS );
